(ns bitbucket-compat.main
  "Kotodama WASM entrypoint for the Bitbucket clean-room actor (L5) — Clojure port.
  ...
  facts (`bitbucket.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bitbucket")
(def tier "L5")
...
(def entity-specs
  [{:entity "PullRequest" :plural "pullrequests" :id-prefix "bitbucke_pul"
    :fields [:title :state :draft :createdOn]         :required [:title]
    :coerce {:draft :bool}         :refs {}}
   {:entity "Repository" :plural "repositories" :id-prefix "bitbucke_rep"
    :fields [:name :fullName :scm :isPrivate :uuid :createdOn] :required [:name :fullName]
    :coerce {:isPrivate :bool}              :refs {}}
   {:entity "Workspace" :plural "workspaces" :id-prefix "bitbucke_wor"
    :fields [:name :slug :uuid :isPrivate :createdOn]    :required [:name :slug]
    :coerce {:isPrivate :bool}              :refs {}}
   {:entity "Project" :plural "projects" :id-prefix "bitbucke_pro"
    :fields [:name :key :uuid :isPrivate :createdOn]     :required [:name :key]
    :coerce {:isPrivate :bool}              :refs {}}
   {:entity "Issue" :plural "issues" :id-prefix "bitbucke_iss"
    :fields [:title :state :priority :kind :createdOn] :required [:title]
    :coerce {}                      :refs {}}
   {:entity "Commit" :plural "commits" :id-prefix "bitbucke_com"
    :fields [:hash :message :date :summary :author]      :required [:hash :message]
    :coerce {}                              :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"bitbucket.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bitbucket-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bitbucket-compat/ready)
